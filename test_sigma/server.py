...
def get_edges_and_ajd_lists(initial_list, db, directional_edges=True):
    ...
    for rel in relationships:
        ...
        edge_id = rel['edge_id']
        rels.append({"source": index_source, "target": index_target, "id": edge_id, "edge_type": rel_type})
...
def get_graph():
    ...
    for record in results:
        x = x + 1
        nodes.append({"id": record["node_id"], "code": record["code"], "title": record["course"], "label": record["course"], 'x':50, 'y':x, 'size':0.0001})
    ...
    return Response(dumps({"nodes": nodes, "edges": []}),
